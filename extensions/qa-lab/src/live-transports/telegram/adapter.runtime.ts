...
export async function createTelegramQaTransportAdapter(context: FactoryContext) {
  ...
  const poll = async () => {
    for (;;) {
      ...
      const updates = await telegramLive.callTelegramApi<TelegramUpdate[]>(
        runtimeEnv.driverToken,
        "getUpdates",
      ...
      for (const update of updates) {
        ...
        if (update.edited_message && existingMessageId && context.state.editMessage) {
          await context.state.editMessage({
        ...
        const outbound = await context.state.addOutboundMessage({
          accountId: options.sutAccountId?.trim() || "sut",
