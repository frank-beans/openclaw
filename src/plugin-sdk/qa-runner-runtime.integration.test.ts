  ...
  it("loads an activated qa runner from a linked plugin path without a bundled install fallback", async () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "  {",
        '    commandName: "linked",',
        '    adapterFactory: { id: "linked", matches() { return true; }, async create(context) { return context.createAdapter({ id: "linked", label: "Linked", accountId: "sut", requiredPluginIds: [], supportedActions: [], async sendInbound(input) { return await context.state.addInboundMessage(input); }, createGatewayConfig() { return {}; }, async waitReady() {}, buildAgentDelivery({ target }) { return { channel: "linked", to: target, replyChannel: "linked", replyTo: target }; }, async handleAction() {}, createReportNotes() { return []; } }); } },',
        "    register() {}",
        "  }",
        "];",
      ].join("\n"),
