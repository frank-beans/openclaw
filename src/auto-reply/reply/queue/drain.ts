...
function consumeQueueSummaryDelivery(
  ...
  preserveLifecycleSource?: FollowupRun,
): void {
  ...
  for (const source of delivery.sources) {
    ...
    if (source !== preserveLifecycleSource) {
    ...
    protectedSources.length > 1
      ? () => {
          ...
          consumeQueueSummaryDelivery(
            ...
            resolveAggregateOwner(protectedSources),
          );
        }
      : undefined;
  try {
    try {
      ...
      if (!admitted && !isFollowupRunDeferredError(err)) {
    ...
    ...(params.onAdmitted
      ? {
          queuedLifecycle: {
            ...
            onComplete: () => {
              const owner = resolveAggregateOwner(params.sources);
              if (admitted && owner) {
                completeFollowupRunLifecycle(owner);
              }
            },
          },
        }
                ...
                ...(activeGroupItems.length > 1
                  ? {
                      queuedLifecycle: {
                        ...
                        onComplete: () => {
                          if (admitted && aggregateOwner) {
                            completeFollowupRunLifecycle(aggregateOwner);
                          }
                        },
                      },
                    }
