...
export function trimSummaryElisionsToCap(queue: SummaryElisionCapState): void {
  let sourceCount = queue.summaryElisions.reduce((count, entry) => count + entry.sources.length, 0);
  while (sourceCount > queue.cap) {
    ...
    if (!evicted) {
      // Active delivery sources add at most one queue-cap-sized temporary set.
      return;
    }
  }
}
